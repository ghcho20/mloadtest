...
def init_config():
    ...
    print(f'Config: doc_size={doc_size}, docs_per_batch={docs_per_batch}')
    return (doc_size, docs_per_batch)

class InsertUser(MongoUser):
    """
    User class for load testing MongoDB operations.
    """
    wait_time = between(0.0, 0.0)
    doc_size, docs_per_batch = init_config()
...
class FindUser(MongoUser):
    """
    User class for load testing MongoDB find operations.
    """
    wait_time = between(0.0, 0.0)
    ...
    def on_start(self):
        ...
        Initialize the user by connecting to the MongoDB cluster.
        """
        self.coll, _ = self.ensure_collection(DEFAULTS['COLLECTION_OLTP'])

    @mongodb_task(weight=1)
    def find(self):
        filters = random.sample(range(1, 65_536), 5)
